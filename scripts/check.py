        ...
        with conn.cursor() as cur:
            ...
            for table_name in sorted(tables):
                try:
                    ...
                    if empty_count > 0:
                        print(
                            f"  [WARN] 空几何: {empty_count:,} ({empty_count/total*100:.1f}%)"
                        )
                    if invalid_count > 0:
                        print(
                            f"  [ERROR] 无效几何: {invalid_count:,} ({invalid_count/total*100:.1f}%)"
                        )
            ...
            if total_records > 0:
                print(
                    f"  空几何: {total_empty:,} ({total_empty/total_records*100:.1f}%)"
                )
                print(
                    f"  无效几何: {total_invalid:,} ({total_invalid/total_records*100:.1f}%)"
                )
            print("=" * 60)
...
示例:
  ...
  python scripts/check.py --all
    ...
    parser.add_argument(
        "--connection", "-c", action="store_true", help="检查数据库连接和PostGIS"
    )
    parser.add_argument(
        "--layers", "-l", metavar="GDB_PATH", help="检查GDB文件中的图层信息"
    )
    parser.add_argument(
        "--geometry", "-g", action="store_true", help="检查几何数据质量"
    )
    parser.add_argument("--all", "-a", action="store_true", help="执行所有检查")
