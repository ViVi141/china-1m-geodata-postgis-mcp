...
def step1_parse_tile(gdb_path: str, output_dir: str = "analysis", reference_tile: str = None) -> str:
    """
    步骤1：解析图幅结构
    ...
    if not reference_tile:
        ...
        reference_tile = gdb_name[:3] if len(gdb_name) >= 3 and gdb_name[0] in ['F', 'G'] else gdb_name
...
def step2_create_schema(analysis_file: str, srid: int = 4326, force: bool = False) -> bool:
    """
    步骤2：创建统一表结构
...
def step3_import_data(gdb_dir: str = ".", srid: int = 4326, batch_size: int = 1000, skip_invalid: bool = True) -> dict:
    """
    步骤3：导入所有图幅数据
...
示例用法：
  ...
  python scripts/setup_unified_database.py --srid 4326 --batch-size 2000
    ...
    parser.add_argument(
        ...
        help='强制重新创建表（会删除已存在的表）'
    )
    parser.add_argument(
        '--skip-parse', action='store_true',
        help='跳过解析步骤（使用已有分析结果）'
    )
    parser.add_argument(
