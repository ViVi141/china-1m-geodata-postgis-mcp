@echo off
REM Windows批处理脚本 - 启动PostgreSQL/PostGIS容器
...
docker images postgis/postgis --format "{{.Repository}}:{{.Tag}}" | findstr postgis >nul
if errorlevel 1 (
...
REM 初始化PostGIS扩展
echo 初始化PostGIS扩展...
docker exec geodata-postgres psql -U postgres -d gis_data -c "CREATE EXTENSION IF NOT EXISTS postgis;" 2>nul
