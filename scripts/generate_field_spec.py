        ...
        for layer_info in sorted(categories[category], key=lambda x: x["table_name"]):
            ...
            for field_name, field_info in sorted(fields.items()):
                ...
                field_desc = get_field_description(
                    layer_code, field_name, field_info, stats
                )
                field_type = field_info.get("type", "")
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "用法: python scripts/generate_field_spec.py <analysis_json> [output_file]"
        )
        print(
            "  示例: python scripts/generate_field_spec.py analysis/F49_schema.json docs/FIELD_SPEC.md"
        )
        sys.exit(1)
