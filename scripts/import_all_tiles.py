        ...
        with fiona.open(gdb_path, layer=layer_name) as src:
            ...
            with conn.cursor() as cur:
                ...
                data_columns = [col for col in existing_columns 
                              if col not in ['id', 'geom', 'tile_code', 'created_at', 'updated_at']]
                    ...
                    try:
                        ...
                        for db_col in mapped_db_columns:
                            ...
                            if gdb_field:
                                ...
                                value = None
                                logger.warning(f"    警告: 字段 {db_col} 找不到对应的GDB字段")
                        ...
                        if len(batch) >= batch_size:
                            try:
                                ...
                                if count == len(batch) and len(batch) > 0:
                                    ...
                                    logger.debug(f"    第一批数据示例: geom长度={len(str(first_record[0]))}, tile_code={first_record[1]}, 其他字段数={len(first_record)-2}")
                                    if len(first_record) > 3:
                                        logger.debug(f"    pac={first_record[2]}, name={first_record[3] if len(first_record) > 3 else 'N/A'}")
                                ...
                                if current_time - last_log_time >= 5 or processed % 1000 == 0:
                                    speed = count / elapsed if elapsed > 0 else 0
                                    logger.info(f"    已处理: {processed:,} 条 - 已导入 {count:,} 条 - 速度: {speed:.0f} 条/秒")
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                if not cur.fetchone()[0]:
                    ...
                    logger.warning(f"  请先运行: python scripts/create_unified_schema.py")
                    skipped_count += 1
                    continue
            ...
            if layer_empty:
                ...
                logger.info(f"  [SKIP] 图层 {layer_name} 为空（0条记录） - 耗时 {elapsed:.2f}秒")
                skipped_count += 1
                continue
            
            logger.info(f"  → 导入到表: {table_name}")
            count = import_layer_data(
                gdb_path, layer_name, table_name, tile_code,
                conn, srid, batch_size, skip_invalid
            )
            ...
            if count > 0:
                ...
                error_count += 1
                logger.warning(f"  ✗ 图层 {layer_name} 导入失败（源数据有记录，但导入0条） - 耗时 {elapsed:.2f}秒")
        
        except Exception as e:
            elapsed = time.time() - layer_start_time
            error_count += 1
            logger.error(f"  ✗ 导入图层 {layer_name} 时出错: {e} - 耗时 {elapsed:.2f}秒")
            import traceback
...
def main():
    ...
    parser.add_argument(
        '--gdb-dir', '-d',
        default='.',
        help='包含GDB文件的目录（默认: 当前目录）'
    )
    parser.add_argument(
        '--gdb', '-g',
        help='单个GDB文件路径（如果指定，只导入该文件）'
    )
    parser.add_argument(
        '--srid', type=int, default=4326,
        help='坐标系SRID（默认: 4326）'
    )
    parser.add_argument(
    ...
    # 连接数据库
    print(f"\n连接数据库: {db_config.get('host', 'localhost')}:{db_config.getint('port', 5432)}/{db_config.get('database')}")
    try:
