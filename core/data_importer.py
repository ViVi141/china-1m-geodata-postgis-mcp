...
class DataImporter:
    ...
    def _detect_format(self, data_path: str) -> str:
        ...
        if path.is_dir():
            # 检查是否是GDB目录
            if path.suffix == ".gdb" or (
                path.is_dir() and any(path.glob("*.gdbtable"))
            ):
                return "gdb"
    ...
    async def list_tables(
        ...
        try:
            with conn.cursor() as cur:
                ...
                return {"tables": geo_tables, "total": len(geo_tables)}
    ...
    async def execute_sql(
        ...
        for keyword in forbidden_keywords:
            ...
            if re.search(pattern, sql_clean):
                raise ValueError(
                    f"不允许执行包含 {keyword} 的SQL语句，只允许SELECT查询"
                )
    ...
    def _get_table_info(self, table_name: str) -> Dict[str, Any]:
        ...
        return {"description": "", "category": "", "layer_code": ""}

    def _get_field_descriptions(
        self, table_name: str, spec_loader: SpecLoader
    ) -> Dict[str, str]:
        """
        ...
        try:
            ...
            for row in cur.fetchall():
                ...
                description = field_descriptions.get(
                    col_name, "字段说明请查看docs/FIELD_SPEC.md"
                )
                columns_info.append(
