            ...
            try:
                ...
                try:
                    ...
                    count = self._import_layer(
                        ...
                        conn, srid, batch_size, skip_invalid, create_indexes
                    )
                    ...
                    elapsed = time.time() - layer_start_time
                    error_count += 1
                    logger.error(f"  ✗ 图层 {layer_name} 导入时出错: {e} - 耗时 {elapsed:.2f}秒")
                    import traceback
    ...
    def _import_layer(
        ...
        try:
            with fiona.open(gdb_path, layer=layer_name) as src:
                ...
                # 确保表存在
                if not self._create_table_if_not_exists(conn, table_name, layer_schema, srid, create_indexes):
                ...
                with conn.cursor() as cur:
                                ...
                                try:
                                    ...
                                    if current_time - last_log_time >= 5 or processed % 1000 == 0:
                                        if total_features:
                                            ...
                                            logger.info(f"    进度: {processed:,}/{total_features:,} ({progress_pct:.1f}%) - 已导入 {count:,} 条 - 速度: {speed:.0f} 条/秒")
                                        else:
                                            speed = count / elapsed if elapsed > 0 else 0
                    ...
                    if total_features:
                        logger.info(f"    处理完成: {processed:,}/{total_features:,} 条记录")
                    else:
            ...
            with conn.cursor() as cur:
                ...
                if not table_exists:
                    ...
                    try:
                        ...
                        if not cur.fetchone():
                            cur.execute(f"ALTER TABLE public.{table_name} ADD COLUMN tile_code VARCHAR(10);")
                            conn.commit()
                            
                            if create_indexes:
                                try:
                                    cur.execute(f"CREATE INDEX IF NOT EXISTS {table_name}_tile_code_idx ON public.{table_name} (tile_code);")
                                    conn.commit()
                                except Exception:
                                    conn.rollback()
                    except Exception as e:
